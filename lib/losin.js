...
var Losin = module.exports = function(socketAdapter, config) {
    ...
    /**
     * On socket close, clear all timeouts to 
     * get rid of never got responses
     */
    this.socketAdapter.onClose(function() {
        _.each(self.timeouts, clearTimeout);
        self.timeouts = [];
    });
};
...
Losin.prototype.sendRequest = function(name, body, callback) {
    ...
    self.socketAdapter.once(respName, function (resp) {
        ...
        self.timeouts = _.remove(self.timeouts, timeout);
